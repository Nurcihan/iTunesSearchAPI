...
final class ContentListViewModel{
    ...
    // MARK: - Public Methods
     ...
     func getDataFromService(text: String) {
        ...
        let path = iTunesAPI.shared.getServiceURL(word: trimmedText, itemsPerPage: itemsPerPage, offset: itemsPerPage * page)
        isLoading = true;
        iTunesAPI.shared.getContentResponseWithSearch(searchLink: path) { sectionList, count in
    ...
    func contentAtSectionAndIndex(_ section :Int, _ index :Int) -> ModelCollectionCellViewModel{
        let section = self.searchSegmentList[section]
        let content = SegmentViewModel(segment: section).contentList[index]
        return ModelCollectionCellViewModel(content: content)
    }
...
// MARK: - Delegate
protocol ContentListViewModelDelegate: NSObject {
